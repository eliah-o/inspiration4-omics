...
# viral metag/metat
viral_metag = read.csv("GenBank_viral_xtree_abundances/genbank-viral_.02_.0.015_metagenomics_species_ra.tsv",check.names=F,sep='\t')
viral_metat = read.csv('GenBank_viral_xtree_abundances/genbank-viral_.02_.0.015_metatranscriptomics_species_ra.tsv',check.names=F,sep='\t')
...
gene_metag = read.table('gene_catalog/gene_catalog90_filtered_relab_metagenomics_subset.tsv',sep='\t',check.names=F,header = T,row.names = 1) 
gene_metag = sanitize_sample_names(gene_metag %>% t %>% as.data.frame(check.names=F))

gene_metat = read.table('gene_catalog/gene_catalog90_filtered_relab_metatranscriptomics_subset.tsv',sep='\t',check.names=F,header = T,row.names = 1)
gene_metat = gene_metat %>% t %>% as.data.frame(check.names=F)

# get the regression data
fortrendline = read.csv('SUPPTABLE_regression_output.csv') 
...
write.csv(bind_rows(output3),paste('LASSO_OUT_METAGENOMIC','_',args[[1]],sep=''))
...
write.csv(bind_rows(output3),paste('LASSO_OUT_METATRANSCRIPTOMIC','_',args[[1]],sep=''))
